...
export default {
  ...
  propertiesManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.address);
  },
  propertiesOneListFormatter(val) {
    if (!val) return '';
    return val.address;
  },
  propertiesManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.address };
    });
  },
  propertiesOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.address, id: val.id };
  },

  reportsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.title);
  },
  reportsOneListFormatter(val) {
    if (!val) return '';
    return val.title;
  },
  reportsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.title };
    });
  },
  reportsOneListFormatterEdit(val) {
    if (!val) return '';
